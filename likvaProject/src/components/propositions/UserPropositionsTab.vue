	...
	import Vuex from 'vuex'
  	...
  	export default {
    	...
    	mounted () {
    		...
    		this.userPropositionsResource.get({userEmail: this.userInfos.email}).then(response => {
    			//	if server answer
    			this.myPropositions = response.body.props
    			this.myProposition.forEach(proposition => {
    				proposition.votersNumber = 12 // To update with an api asker
    				if (Date.parse(proposition.date) > Date.now()){
    					//  Proposition done
    					this.reslutResource.get({slug: proposition.slug, idProposition: proposition._id}).then(res => {
    						// If server answer
    						proposition.result = res.body.verdict
    						doneProposition.push(proposition)
    					}, _ => {
    					...
    					inProgressPropostions.push(proposition)
    				}
    			})
    		}, _ => {
    			//	if server doesn't answer
